...
Cope events so that they can be written to a MIDI
file. The global variable *rs* is defined here.
...
(setq *states* (realise-states *initial-states* *stm* 5))
#| These functions help to show the process, but it's
possible to go straight for the function states2cope-
events.
(setq *state-durs* (state-durations *states*))
(setq *unique-times* (cons 0 (fibonacci-list *state-durs*)))
(setq *half-states* (create-MIDI-note-numbers *states* 48))
|#
(setq *cope-events* (states2cope-events *states* 48))
(setq *events* (scale-cope-events-by-factor 1 *cope-events*))
(saveit) 
|#
...
A random, equiprobable choice is made between elements
of a list. |#
...
The nth element of the list returned is the sum of the
previous n-1 elements, with the convention that a sum
...
state-note2cope-event is applied. |#

(defun half-state2cope-events
       (j half-states
  ...
  (if (equal i n) ()
    (let ((tie-type
	 ...
	 (list
	  (state-note2cope-event i j half-states
				 state-durs
				 unique-times)))
       (half-state2cope-events
...
(defun realise-states (initial-states transition-matrix
		       &optional (count 5)
		        (initial-state
...
The ontimes and durations of cope-events are scaled
up or down by a constant factor. |#

(defun scale-cope-events-by-factor (factor cope-events)
  (let ((current-event (first cope-events)))
    (if (null cope-events) ()
      (cons (list
...
a so-called `cope-event', meaning we find its ontime
...
(defun state-note2cope-event (i j half-states
			      state-durs unique-times)
  (let* ((current-state (nth j half-states))
	 (note-ontime (nth j unique-times))
	 (note-number (nth i (first
			      (first current-state))))
	 (offtime-state
	  (index-of-offtime
	...
	(nth offtime-state state-durs))))
     ...
     (fifth (nth i (fourth (second current-state)))))))
