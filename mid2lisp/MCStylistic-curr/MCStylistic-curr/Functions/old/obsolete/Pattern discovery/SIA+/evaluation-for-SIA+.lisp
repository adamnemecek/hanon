...
\noindent The aim of these functions is to provide
support for the implementation of COSIATEC (also 
possibly Jamie Forth's algorithm). |#
...
to three, and normalised linearly to [0, 1]. Then the
lists are multiplied element-wise, and the resulting
list is searched for the argument of the maximum
element. |#
...
(defun argmax-of-threeCs
	...
	(to-interrogate
	 (multiply-two-lists
	  (normalise-0-1
	   (nth-list-of-lists
	    0 threeCs-list-of-lists))
	  (multiply-two-lists
	   (normalise-0-1
	    (nth-list-of-lists
	     1 threeCs-list-of-lists))
	   (normalise-0-1
	    (nth-list-of-lists
	     2 threeCs-list-of-lists))))))
  (second (max-argmax to-interrogate)))
|#
...
translation is returned, and nil otherwise.

This function is used for checking the output of the
COSIATEC algorithm, as it uses assoc. |#
...
This function is used for checking the output of the
SIA algorithm, as it uses rassoc. |#
...
This function is used for checking the output of the
COSIATEC-mod-2nd-n algorithm, as it uses assoc (when
the dataset has been projected modulo n). |#
...
This function is used for checking the output of the
SIA-mod-2nd-n algorithm, as it uses rassoc and when
the dataset has been projected modulo n. |#
