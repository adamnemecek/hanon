...
algorithm SIACT-delta to Scarlatti's Sonata in C
Minor L10 (K84). |#
...
#| Step 4 - Run SIACT-delta on projection
(1 1 0 1 0). |#
(progn
  (setq
   SIA-1-1-0-1-0
  ...
  (time
   (compactness-trawler-delta
    ...
    (concatenate
     ...
     "/Applications/CCL/Lisp code/Pattern discovery"
     "/SIACT/Write to files"
     "/L 10 (1 1 0 1 0) CT-delta 1.txt")
    *delta* *cardina-thresh*)))
; 30.246576 seconds.
  ...
  (time
   (setq
    ...
    (evaluate-variables-of-patterns2hash
     SIACT-delta-1-1-0-1-0 dataset-1-1-0-1-0))))
; 2818.918000 seconds.
...
(progn
  ...
  (time
   (setq
    hash-1-0-1-1-0
    (evaluate-variables-of-patterns2hash
     SIACT-delta1-0-1-1-0 dataset-1-0-1-1-0))))
; 779.912230 seconds.
